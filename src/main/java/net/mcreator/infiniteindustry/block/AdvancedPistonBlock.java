...
public class AdvancedPistonBlock extends InfiniteIndustryElements.ModElement {
	...
	public void registerTileEntity(RegistryEvent.Register<TileEntityType<?>> event) {
		event.getRegistry().register(TileEntityType.Builder.create(CustomTileEntity::new, block).build(null).setRegistryName("advancedpistonbody"));
	}
	public static class CustomBlock extends Block {
		...
		public VoxelShape getShape(BlockState state, IBlockReader world, BlockPos pos, ISelectionContext context) {
			switch ((Direction) state.get(FACING)) {
				case SOUTH :
				default :
					return VoxelShapes.create(1D, 0D, 0.75D, 0D, 1D, 0D);
				case NORTH :
					return VoxelShapes.create(0D, 0D, 0.25D, 1D, 1D, 1D);
				case WEST :
					return VoxelShapes.create(0.25D, 0D, 1D, 1D, 1D, 0D);
				case EAST :
					return VoxelShapes.create(0.75D, 0D, 0D, 0D, 1D, 1D);
				case UP :
					return VoxelShapes.create(0D, 0.75D, 0D, 1D, 0D, 1D);
				case DOWN :
					return VoxelShapes.create(0D, 0.25D, 1D, 1D, 1D, 0D);
			}
		}
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(AdvancedPistonBlockBlock.block, (int) (1)));
		}
		...
		public void neighborChanged(BlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos, boolean moving) {
			...
			if (world.getRedstonePowerFromNeighbors(new BlockPos(x, y, z)) > 0) {
			...
			} else {
				{
					...
					$_dependencies.put("z", z);
					$_dependencies.put("world", world);
					AdvancedPistonRedstoneOffProcedure.executeProcedure($_dependencies);
				}
			}
		}
		...
		public boolean removedByPlayer(BlockState state, World world, BlockPos pos, PlayerEntity entity, boolean willHarvest, IFluidState fluid) {
			...
			int z = pos.getZ();
			{
				...
				$_dependencies.put("z", z);
				$_dependencies.put("world", world);
				AdvancedPistonRodBlockDestroyedByPlayerProcedure.executeProcedure($_dependencies);
			}
			return retval;
		}
	...
	public static class CustomTileEntity extends LockableLootTileEntity {
		...
		public ITextComponent getDefaultName() {
			return new StringTextComponent("advancedpistonbody");
		}
		...
		public ITextComponent getDisplayName() {
			return new StringTextComponent("Advanced Piston");
		}
